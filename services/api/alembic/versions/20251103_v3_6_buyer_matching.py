...
def upgrade():
    # Check if buyers table already exists (idempotent)
    if not op.get_bind().dialect.has_table(op.get_bind(), "buyers"):
        op.create_table(
            "buyers",
    ...
    if not op.get_bind().dialect.has_table(op.get_bind(), "deal_briefs"):
        op.create_table(
            "deal_briefs",
            sa.Column("id", sa.Integer, primary_key=True),
