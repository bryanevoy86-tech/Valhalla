...
async def lifespan(app: FastAPI):
    ...
    # Initialize community module tables (direct SQLAlchemy, bypasses Alembic)
    try:
        print("LIFESPAN: initializing community module tables")
        log.info("LIFESPAN: initializing community module tables")
        from app.core.db import Base, engine, SessionLocal
        from app.models import community as _cm  # Import to register models
        from app.seeds import community_seed
        
        # Create tables
        Base.metadata.create_all(engine)
        print("LIFESPAN: community tables created")
        log.info("LIFESPAN: community tables created")
        
        # Seed test data if needed
        db = SessionLocal()
        try:
            community_seed.seed_community(db)
            print("LIFESPAN: community seed data loaded")
            log.info("LIFESPAN: community seed data loaded")
        finally:
            db.close()
    except Exception as e:
        print(f"LIFESPAN: Warning during community initialization: {type(e).__name__}: {e}")
        log.warning("Community initialization had an issue (may already exist): %s", e)
...
ROUTERS = [
    ...
    RouterSpec("community", "app.routers.community", required=False),
]
