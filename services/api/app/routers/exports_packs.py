...
async def generate_eia_export(
    ...
    """
    try:
        return generate_eia_pack(db, year, month)
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Failed to generate EIA pack: {str(e)}")
...
async def generate_accountant_export(
    ...
    """
    try:
        return generate_accountant_pack(db, year, month)
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Failed to generate accountant pack: {str(e)}")
...
async def generate_legal_export(
    ...
    """
    try:
        return generate_legal_pack(db, year, month)
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Failed to generate legal pack: {str(e)}")
...
async def generate_appointment_export(
    ...
    """
    try:
        return generate_appointment_pack(db, year, month)
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Failed to generate appointment pack: {str(e)}")
...
async def ensure_close_appointment_eia(
    ...
    Returns success whether month was just locked or was already locked.
    ...
    Returns:
        MonthCloseResponse with lock receipt
    """
    try:
        result = close_eia_month(db, year, month, locked_by)
        # Result is a dict; both first close and idempotent second close succeed
        return result
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Failed to ensure close: {str(e)}")
...
async def get_month_status(
    ...
    try:
        from app.services.eia_export import EIAMonthManager
...
def _get_safe_export_path(package_type: str, year: int, month: int) -> Path:
    ...
    """
    # Get the app directory: __file__ = app/routers/exports_packs.py
    # So parent.parent = app/ (same as service uses)
    router_file = Path(__file__)
    app_dir = router_file.parent.parent  # app/
    base_exports_dir = app_dir / "generated_exports"
