from __future__ import annotations

import json, os
from datetime import datetime, timezone
from typing import Any, Dict, List

DATA_DIR = os.path.join("backend", "data", "family_payroll")
PEOPLE = os.path.join(DATA_DIR, "people.json")
ENTRIES = os.path.join(DATA_DIR, "entries.json")

def _utcnow_iso() -> str:
    return datetime.now(timezone.utc).isoformat()

def _ensure():
    os.makedirs(DATA_DIR, exist_ok=True)
    for p in (PEOPLE, ENTRIES):
        if not os.path.exists(p):
            with open(p, "w", encoding="utf-8") as f:
                json.dump({"updated_at": _utcnow_iso(), "items": []}, f, indent=2)

def list_people() -> List[Dict[str, Any]]:
    _ensure()
    with open(PEOPLE, "r", encoding="utf-8") as f:
        return json.load(f).get("items", [])

def save_people(items: List[Dict[str, Any]]) -> None:
    _ensure()
    tmp = PEOPLE + ".tmp"
    with open(tmp, "w", encoding="utf-8") as f:
        json.dump({"updated_at": _utcnow_iso(), "items": items}, f, indent=2, ensure_ascii=False)
    os.replace(tmp, PEOPLE)

def list_entries() -> List[Dict[str, Any]]:
    _ensure()
    with open(ENTRIES, "r", encoding="utf-8") as f:
        return json.load(f).get("items", [])

def save_entries(items: List[Dict[str, Any]]) -> None:
    _ensure()
    tmp = ENTRIES + ".tmp"
    with open(tmp, "w", encoding="utf-8") as f:
        json.dump({"updated_at": _utcnow_iso(), "items": items}, f, indent=2, ensure_ascii=False)
    os.replace(tmp, ENTRIES)
